...
export function computeCoupledCapitalResourceSeries(
  sourceFrame: RuntimeStateFrame,
  prepared: RuntimePreparation,
  constantsUsed: ConstantMap,
  ...
  if (
    ...
    throw new Error(
      "Fixture-backed runtime cannot derive coupled capital-resource values because the required lookups are incomplete.",
    );
  }
  ...
  const ioDefinition = createCapitalIoDerivedDefinition();
  const iopcDefinition = createIopcDerivedDefinition();
  const fcaorDefinition = createFcaorDerivedDefinition(
    constantsUsed,
    fcaor1Lookup,
    fcaor2Lookup,
  );
  ...
  const icdrDefinition = createIcdrDerivedDefinition();
  const scdrDefinition = createScdrDerivedDefinition();
  const nrufDefinition = createNrufDerivedDefinition(
    constantsUsed,
    prepared.request.pyear,
  );
  ...
  for (let index = 0; index < sourceFrame.time.length; index += 1) {
    ...
    values.fcaor =
      observedFcaorValue ??
      fcaorDefinition.derive(createCapitalObservation(index, time, values));
