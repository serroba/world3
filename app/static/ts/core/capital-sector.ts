...
export type CapitalOrderedSeries = {
  io: Float64Array;
  iopc: Float64Array;
  so: Float64Array;
  sopc: Float64Array;
  [CAPITAL_HIDDEN_SERIES.alic]: Float64Array;
  [CAPITAL_HIDDEN_SERIES.alsc]: Float64Array;
  [CAPITAL_HIDDEN_SERIES.cuf]: Float64Array;
  [CAPITAL_HIDDEN_SERIES.fioac]: Float64Array;
  [CAPITAL_HIDDEN_SERIES.fioai]: Float64Array;
  [CAPITAL_HIDDEN_SERIES.ic]: Float64Array;
  [CAPITAL_HIDDEN_SERIES.icdr]: Float64Array;
  [CAPITAL_HIDDEN_SERIES.icor]: Float64Array;
  [CAPITAL_HIDDEN_SERIES.fioas]: Float64Array;
  [CAPITAL_HIDDEN_SERIES.icir]: Float64Array;
  [CAPITAL_HIDDEN_SERIES.isopc]: Float64Array;
  [CAPITAL_HIDDEN_SERIES.sc]: Float64Array;
  [CAPITAL_HIDDEN_SERIES.scor]: Float64Array;
  [CAPITAL_HIDDEN_SERIES.scdr]: Float64Array;
  [CAPITAL_HIDDEN_SERIES.scir]: Float64Array;
};

export type CoupledCapitalResourceSeries = CapitalOrderedSeries & {
  nr: Float64Array;
  [RESOURCE_HIDDEN_SERIES.nruf]: Float64Array;
  [RESOURCE_HIDDEN_SERIES.pcrum]: Float64Array;
  [RESOURCE_HIDDEN_SERIES.nrRate]: Float64Array;
};
...
export function createAlicDerivedDefinition(
  constantsUsed: ConstantMap,
  policyYear = DEFAULT_CAPITAL_POLICY_YEAR,
): RuntimeDerivedDefinition {
  return {
    ...
    derive: (observation: RuntimeObservation) =>
      clipAtPolicyYear(
        constantsUsed.alic1 ?? 14,
        constantsUsed.alic2 ?? 14,
        observation.time,
        policyYear,
      ),
  };
...
export function createAlscDerivedDefinition(
  constantsUsed: ConstantMap,
  policyYear = DEFAULT_CAPITAL_POLICY_YEAR,
): RuntimeDerivedDefinition {
  return {
    ...
    derive: (observation: RuntimeObservation) =>
      clipAtPolicyYear(
        constantsUsed.alsc1 ?? 20,
        constantsUsed.alsc2 ?? 20,
        observation.time,
        policyYear,
      ),
  };
...
export function createIcdrDerivedDefinition(): RuntimeDerivedDefinition {
  return {
    ...
    derive: (observation: RuntimeObservation) => {
      ...
      if (ic === undefined) {
        throw new Error(
          "Fixture-backed runtime cannot derive '__icdr' because the source variable '__ic' is missing.",
        );
      }
      if (alic === undefined || alic === 0) {
        throw new Error(
          "Fixture-backed runtime cannot derive '__icdr' because the source variable '__alic' is missing or zero.",
        );
      }
      return ic / alic;
    },
...
export function createScdrDerivedDefinition(): RuntimeDerivedDefinition {
  return {
    ...
    derive: (observation: RuntimeObservation) => {
      ...
      if (sc === undefined) {
        throw new Error(
          "Fixture-backed runtime cannot derive '__scdr' because the source variable '__sc' is missing.",
        );
      }
      if (alsc === undefined || alsc === 0) {
        throw new Error(
          "Fixture-backed runtime cannot derive '__scdr' because the source variable '__alsc' is missing or zero.",
        );
      }
      return sc / alsc;
    },
...
export function extendCapitalSourceVariables(
  ...
  const canUseNativeResourceFeedbackInCapital =
...
function constantsUsedHasCapitalStockSeeds(constantsUsed: ConstantMap): boolean {
  return Boolean(
    constantsUsed.ici !== undefined &&
...
export function populateCapitalNativeSupportSeries(
  ...
  if (canUseNativeCapitalOrdering) {
    const orderedSeries = computeCapitalOrderedSeries(
      sourceFrame,
      prepared,
      constantsUsed,
    );
  ...
  sourceSeries.set(
    CAPITAL_HIDDEN_SERIES.alic,
    deriveSeriesValues(
      sourceFrame,
      createAlicDerivedDefinition(constantsUsed),
    ),
  );
  sourceSeries.set(
    CAPITAL_HIDDEN_SERIES.alsc,
    deriveSeriesValues(
      sourceFrame,
      createAlscDerivedDefinition(constantsUsed),
    ),
  );
...
function requireSeries(
  ...
  if (!values) {
    throw new Error(
      `Fixture-backed runtime cannot derive ordered capital values because the source variable '${variable}' is missing.`,
    );
  }
  return values;
}

function createCapitalObservation(
  index: number,
  time: number,
...
export function computeCapitalOrderedSeries(
  sourceFrame: RuntimeStateFrame,
  prepared: RuntimePreparation,
  constantsUsed: ConstantMap,
  ...
  if (
    ...
    throw new Error(
      "Fixture-backed runtime cannot derive ordered capital values because the capital lookups are incomplete.",
    );
  }
  ...
  if (!fcaorSeries && !canDeriveNativeFcaor) {
    throw new Error(
      "Fixture-backed runtime cannot derive ordered capital values because 'fcaor' or native resource feedback inputs are missing.",
    );
  }
  ...
  const fcaorDefinition =
    canDeriveNativeFcaor && fcaor1Lookup && fcaor2Lookup
  ...
  for (let index = 0; index < sourceFrame.time.length; index += 1) {
    ...
    if (observedFcaorValue !== undefined) {
      ...
      values.nr = nrValue;
      values.fcaor = fcaorDefinition.derive(
        createCapitalObservation(index, time, values),
      );
    } else {
      throw new Error(
        "Fixture-backed runtime is missing 'fcaor' and cannot derive it from native resource state.",
      );
    }
...
export const createIndustrialOutputFromCapitalStocksDefinition =
  createCapitalIoDerivedDefinition;
export const createIndustrialOutputPerCapitaDefinition =
  createIopcDerivedDefinition;
export const createServiceOutputDefinition = createSoDerivedDefinition;
export const createServiceOutputFromCapitalStocksDefinition =
...
export const createConsumptionAllocationFractionDefinition =
...
export const createIndustrialCapitalInvestmentRateDefinition =
  createIcirDerivedDefinition;
export const createServiceCapitalInvestmentRateDefinition =
  createScirDerivedDefinition;
