...
export function simulateWorld3(options: World3SimulationOptions): SimulationResult {
  ...
  function computeStep(k: number): void {
    ...
    // State advances
    if (k === 0) {
      p1[0] = p1i; p2[0] = p2i; p3[0] = p3i; p4[0] = p4i;
      ic[0] = ici; sc[0] = sci;
      al[0] = ali; pal[0] = pali; uil[0] = uili;
      lfert[0] = lferti; ppol[0] = ppoli; nr[0] = nri;
    } else {
    ...
    m1[k] = M1(le[k]!); m2[k] = M2(le[k]!);
    m3[k] = M3(le[k]!); m4[k] = M4(le[k]!);
    mat1[k] = p1[k]!*(1-m1[k]!)/15;
    mat2[k] = p2[k]!*(1-m2[k]!)/30;
    mat3[k] = p3[k]!*(1-m3[k]!)/20;
    d1[k] = p1[k]!*m1[k]!; d2[k] = p2[k]!*m2[k]!;
    d3[k] = p3[k]!*m3[k]!; d4[k] = p4[k]!*m4[k]!;
    const fm_k = FM(le[k]!);
    mtf[k] = mtfn * fm_k;
  ...
  const exportMap: Record<string, Float64Array> = {
