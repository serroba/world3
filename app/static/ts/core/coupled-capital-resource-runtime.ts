...
import {
  ...
  type CoupledCapitalResourceSeries,
...
import type { RuntimeObservation, RuntimeStateFrame } from "./runtime-state-frame.js";
...
function requireSeries(
  ...
  if (!values) {
    throw new Error(
      `Fixture-backed runtime cannot derive coupled capital-resource values because the source variable '${variable}' is missing.`,
    );
  }
  return values;
}
