...
function getLookupOrThrow(lookupLibrary, name) {
...
function getConstantOrThrow(constantsUsed, name) {
...
export function extendPollutionSourceVariables(sourceVariables, outputVariables, fixture, lookupLibrary, canUseNativeAgricultureOrdering, canUseNativeNrFlow, needsNativePollutionForPopulation = false) {
    const needsNativePollutionPath = outputVariables.some((variable) => POLLUTION_OUTPUTS.has(variable)) ||
        needsNativePollutionForPopulation;
    const hasAgricultureInputs = canUseNativeAgricultureOrdering ||
        (Boolean(fixture.series.aiph) && Boolean(fixture.series.al));
    const hasResourceInputs = canUseNativeNrFlow || Boolean(fixture.series.pcrum);
    const hasConstants = fixture.constants_used.ppoli !== undefined &&
    ...
    const canUseNativePollutionPath = needsNativePollutionPath &&
...
export function computePollutionOrderedSeries(sourceFrame, prepared, constantsUsed) {
    ...
    const aiph = sourceFrame.series.get(AGRICULTURE_HIDDEN_SERIES.aiph) ??
        sourceFrame.series.get("aiph");
    const pcrum = sourceFrame.series.get(RESOURCE_HIDDEN_SERIES.pcrum) ??
...
export function populatePollutionNativeSupportSeries(sourceFrame, sourceSeries, prepared, constantsUsed, canUseNativePollutionPath) {
    if (!canUseNativePollutionPath) {
        return;
    }
    const orderedSeries = computePollutionOrderedSeries(sourceFrame, prepared, constantsUsed);
    for (const [name, values] of Object.entries(orderedSeries)) {
        sourceSeries.set(name, values);
    }
}
export function maybePopulatePollutionOutputSeries(variable, sourceFrame, outputSeries, fixture, projectedIndices) {
    if (!POLLUTION_OUTPUTS.has(variable)) {
        return false;
    }
    ...
    if (source) {
        outputSeries.set(variable, projectSeriesValues(source.values, projectedIndices, source.name));
        return true;
    }
    return false;
}
