...
function getSeriesValue(sourceSeries, name, index) {
...
export function extendAgricultureSourceVariables(sourceVariables, outputVariables, fixture, lookupLibrary, needsNativeFoodPath = false) {
    const needsAgricultureOrdering = outputVariables.some((variable) => [
        ...
        "mpai",
        "mlymc",
    ].includes(variable)) || needsNativeFoodPath;
    const canUseNativeAgricultureOrdering = needsAgricultureOrdering &&
...
export function computeAgricultureOrderedSeries(sourceFrame, prepared, constantsUsed) {
    ...
    for (let index = 0; index < length; index += 1) {
        ...
        ifpc[index] = clipAtPolicyYear(ifpc1Lookup.evaluate(iopcValue), ifpc2Lookup.evaluate(iopcValue), currentTime, policyYear);
        alai[index] = clipAtPolicyYear(alai1, alai2, currentTime, policyYear);
        lyf[index] = clipAtPolicyYear(constantsUsed.lyf1 ?? 1, constantsUsed.lyf2 ?? 1, currentTime, policyYear);
        lymap[index] = clipAtPolicyYear(lymap1Lookup.evaluate(ioValue / io70), lymap2Lookup.evaluate(ioValue / io70), currentTime, policyYear);
        ...
        if (index === 0) {
            ...
            for (let iteration = 0; iteration < 8; iteration += 1) {
                ...
                fioaa[index] = clipAtPolicyYear(fioaa1Lookup.evaluate(fpcValue / ifpcValue), fioaa2Lookup.evaluate(fpcValue / ifpcValue), currentTime, policyYear);
        ...
        const lyValue = valueAt(ly, index, "ly");
        llmy[index] = clipAtPolicyYear(llmy1Lookup.evaluate(lyValue / ilf), llmy2Lookup.evaluate(lyValue / ilf), currentTime, policyYear);
...
export function populateAgricultureNativeSupportSeries(sourceFrame, sourceSeries, prepared, constantsUsed, canUseNativeFoodPath, canUseNativeAgriculturalAllocation, canUseNativeAgricultureProductivity, canUseNativeAgricultureOrdering = false) {
    if (canUseNativeAgricultureOrdering) {
        const orderedSeries = computeAgricultureOrderedSeries(sourceFrame, prepared, constantsUsed);
