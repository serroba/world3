...
export const WORLD3_COMPARE_METRICS = [
    ...
    "nrfr",
    "le",
].map((key) => resolveWorld3CompareMetric(key));
export function resolveWorld3SimulationPlotVariable(key, definitions = SERIES_BY_KEY) {
    const definition = definitions.get(key);
...
export const WORLD3_SIMULATION_PLOT_VARIABLES = [
    ...
    "fpc",
    "ppolx",
].map((key) => resolveWorld3SimulationPlotVariable(key));
export function buildWorld3VariableMeta() {
    return Object.fromEntries(WORLD3_SERIES_REGISTRY.map((definition) => [
        definition.key,
        {
...
export function buildWorld3ConstantMeta() {
    return Object.fromEntries(WORLD3_CONSTANT_REGISTRY.map((definition) => [
        definition.key,
        {
            full_name: definition.fullName,
            sector: definition.sector,
...
export function buildWorld3SeriesResult(buffers) {
    const series = {};
