...
import { POPULATION_HIDDEN_SERIES, createBirthRateDerivedDefinition, createBirthsDerivedDefinition, createCdrDerivedDefinition, createCmpleDerivedDefinition, createCmiDerivedDefinition, createDcfsDerivedDefinition, createDeathDerivedDefinition, createDtfDerivedDefinition, createFcapcDerivedDefinition, createFceDerivedDefinition, createFieDerivedDefinition, createFmDerivedDefinition, createFrsnDerivedDefinition, createFsafcDerivedDefinition, createFpuDerivedDefinition, createHsapcDerivedDefinition, createLeDerivedDefinition, createLmcDerivedDefinition, createLmfDerivedDefinition, createLmhsDerivedDefinitions, createLmpDerivedDefinition, createMaturationDerivedDefinition, createMortalityDerivedDefinition, createMtfDerivedDefinition, createNfcDerivedDefinition, createPopulationSumDerivedDefinition, createSfsnDerivedDefinition, createTfDerivedDefinition, createTotalDeathsDerivedDefinition, } from "./population-sector.js";
...
function deriveSmoothSeriesValues(input, dt, delay, length = input.length) {
...
function deriveDlinf3SeriesValues(input, dt, delay, length = input.length) {
...
export function populatePopulationNativeSupportSeries(sourceFrame, sourceSeries, prepared, constantsUsed, canUseNativeLifeExpectancy, canUseNativeMortality = false, canUseNativeCohortSupport = false, canUseNativeDeathPath = false, canUseNativePopulationStocks = false) {
    ...
    if (canUseNativeCohortSupport) {
        sourceSeries.set("pop", deriveSeriesValues(sourceFrame, createPopulationSumDerivedDefinition()));
    }
    ...
    const ehspc = deriveSmoothSeriesValues(hsapcValues, prepared.request.dt ?? 1, constantsUsed.hsid ?? 20, sourceFrame.time.length);
    ...
    for (const definition of createLmhsDerivedDefinitions(lmhs1Lookup, lmhs2Lookup, constantsUsed.iphst ?? 1940)) {
    ...
    for (const variable of ["m1", "m2", "m3", "m4"]) {
...
export function populatePopulationBirthNativeSupportSeries(sourceFrame, sourceSeries, prepared, constantsUsed, canUseNativeBirthSupport = false) {
    ...
    sourceSeries.set(POPULATION_HIDDEN_SERIES.aiopc, deriveSmoothSeriesValues(iopcValues, dt, ieat, sourceFrame.time.length));
    sourceSeries.set(POPULATION_HIDDEN_SERIES.diopc, deriveDlinf3SeriesValues(iopcValues, dt, sad, sourceFrame.time.length));
    sourceSeries.set(POPULATION_HIDDEN_SERIES.ple, deriveDlinf3SeriesValues(leValues, dt, lpd, sourceFrame.time.length));
    ...
    for (const definition of [
        ...
        sourceSeries.set(definition.variable, deriveSeriesValues(birthSupportFrame, definition));
    }
    const fcapcValues = sourceSeries.get(POPULATION_HIDDEN_SERIES.fcapc);
    if (!fcapcValues) {
        return;
    }
    sourceSeries.set(POPULATION_HIDDEN_SERIES.fcfpc, deriveDlinf3SeriesValues(fcapcValues, dt, hsid, sourceFrame.time.length));
    birthSupportFrame = {
        request: sourceFrame.request,
        time: sourceFrame.time,
        constantsUsed,
