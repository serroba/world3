...
async function loadResponseBody(requestPath) {
    ...
    return {
        ...
        contentType: MIME_TYPES[path.extname(targetPath)] ?? "application/octet-stream",
    };
}
async function main() {
    ...
    server.listen(options.port, options.host, () => {
        process.stdout.write(`Static server listening on http://${options.host}:${options.port}\n`);
    });
}
main().catch((error) => {
    const message = error instanceof Error ? error.message : String(error);
