...
describe("runtime state frame", () => {
  ...
  test("can derive the native population death path", () => {
    const prepared = prepareRuntime(
      ModelData,
      {
        year_min: 1900,
  ...
  test("can derive the native population stock path", () => {
    const prepared = prepareRuntime(
      ModelData,
      {
        year_min: 1900,
  ...
  test("can derive the native birth path including p1 stepping", () => {
    const prepared = prepareRuntime(
      ModelData,
      {
        year_min: 1900,
    ...
    const result = runtimeStateFrameToSimulationResult(frame);
    const expectations = {
      p1: [1, 1.0194037470997106, 0.9354030535095691],
      b: [0.12529654709971055, 0.1257180733431918, 0.12654756393802175],
      cbr: [12.529654709971055, 13.246280731923632, 14.387989823808349],
      tf: [3.7588964129913167, 3.859123726903247, 3.950871070251402],
      pop: [10, 9.49082054709971, 8.795360956442902],
    } as const;
